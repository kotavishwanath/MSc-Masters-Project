...
class PPGlucoseVC: UIViewController {
    ...
    @IBAction func addMedication(_ sender: Any) {
        ...
        if (medicineName.text != "" && howManyTimes.text != "" && (beforeMealBtn.isSelected || afterMealBtn.isSelected)){
            ...
            if (daysToTake.text == ""){
                glucoseMedication.setValue(1, forKey: "days_to_take")
            }else {
                glucoseMedication.setValue(Int(daysToTake.text!), forKey: "days_to_take")
            }
            do{
    ...
    /**
     Updating all the medicaitions required for the Glucose data to the database
     */
