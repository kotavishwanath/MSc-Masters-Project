...
class PPTemperatureVC: UIViewController {
    ...
    @IBAction func addMedicineButton(_ sender: Any) {
        ...
        if (medicineName.text != "" && timesADay.text != "" && (beforeMealBtn.isSelected || afterMealBtn.isSelected)){
            ...
            if (daysToTake.text == ""){
                tempeartureMedication.setValue(1, forKey: "days_to_take")
            }else {
                tempeartureMedication.setValue(Int(daysToTake.text!), forKey: "days_to_take")
            }
            do{
    ...
    /**
     Updating all the medicaitions required for the temperature data to the database
     */
